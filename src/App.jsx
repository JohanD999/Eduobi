...
const styles = `
  ...
  *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
  ...
  .modal-overlay{position:fixed;inset:0;background:rgba(0,0,0,.75);backdrop-filter:blur(6px);display:flex;align-items:center;justify-content:center;z-index:2000;animation:fadeIn .2s ease}
  .modal-box{background:#131929;border-radius:22px;border:1px solid #243154;padding:32px;width:92%;max-width:460px;animation:popIn .3s ease}
...
const BATCHES = [
  { id: 1, name: "Arjuna 2.0 — JEE 2026", classId: "jee", price: 599, originalPrice: 19999, rating: 4.8, students: "2.1L", instructor: "Anmol Thakur", tag: "BESTSELLER", color: "#FF6B00", icon: "🚀", desc: "Complete JEE prep — 500+ hrs content, DPPs, mock tests & live doubt sessions.", subjects: ["Physics", "Chemistry", "Maths"], duration: "12 months", lectures: 0, tests: 0 },
  { id: 2, name: "Lakshya — NEET 2026", classId: "neet", price: 599, originalPrice: 17999, rating: 4.9, students: "3.2L", instructor: "Dr. Priya Sharma", tag: "TOP RATED", color: "#E94560", icon: "🎯", desc: "Complete NEET batch — NCERT mastery, full mock tests & biology deep dives.", subjects: ["Physics", "Chemistry", "Biology"], duration: "12 months", lectures: 0, tests: 0 },
  { id: 3, name: "Infinity+ Class 12 PCM", classId: 12, price: 599, originalPrice: 12999, rating: 4.7, students: "1.8L", instructor: "Anmol Thakur", tag: "NEW", color: "#4A90E2", icon: "⚡", desc: "Board + JEE combined prep. Smart study plans for Class 12 students.", subjects: ["Physics", "Chemistry", "Maths"], duration: "10 months", lectures: 0, tests: 0 },
  { id: 4, name: "Dropper Batch — JEE 2026", classId: "jee", price: 599, originalPrice: 22999, rating: 4.8, students: "85K", instructor: "Vikram Singh", tag: "DROPPER", color: "#00D4AA", icon: "💪", desc: "Intensive revision for JEE droppers. Full syllabus with advanced problems.", subjects: ["Physics", "Chemistry", "Maths"], duration: "8 months", lectures: 0, tests: 0 },
  { id: 5, name: "Enthusia — Class 11 PCB", classId: 11, price: 599, originalPrice: 10999, rating: 4.6, students: "1.1L", instructor: "Dr. Priya Sharma", tag: "", color: "#9B59B6", icon: "🌟", desc: "Build a strong NEET foundation from Class 11 with a concept-first approach.", subjects: ["Physics", "Chemistry", "Biology"], duration: "10 months", lectures: 0, tests: 0 },
  { id: 6, name: "CUET Master Batch", classId: "cuet", price: 599, originalPrice: 8999, rating: 4.7, students: "65K", instructor: "Rahul Gupta", tag: "POPULAR", color: "#E67E22", icon: "📚", desc: "Score 700+ in CUET. Proven strategy + 300+ practice sets.", subjects: ["Maths", "English", "GK"], duration: "6 months", lectures: 0, tests: 0 },
];
...
const AI_KB = {
  newton: "**Newton's Laws of Motion** 🔬\n\n**1st Law (Inertia):** An object continues in its state of rest or uniform motion unless acted on by an external force.\n\n**2nd Law:** F = ma. Net force = mass × acceleration.\n\n**3rd Law:** Every action has an equal and opposite reaction.\n\n💡 *Memory tip:* \"Lazy objects, Force equals ma, Reactions always pair!\"",
  integration: "**Integration Essentials** ∫\n\n• ∫xⁿ dx = xⁿ⁺¹/(n+1) + C\n• ∫eˣ dx = eˣ + C\n• ∫sin x dx = −cos x + C\n• ∫cos x dx = sin x + C\n• ∫1/x dx = ln|x| + C\n\n**JEE Tips:**\n→ Master substitution & parts first\n→ Integration by parts: ∫u dv = uv − ∫v du\n→ Use LIATE rule for choosing u\n\n🎯 Practice 10 problems daily!",
  periodic: "**Periodic Table Tricks** 🧪\n\n**Group 1 (Alkali Metals):** Li Na K Rb Cs Fr\n→ Mnemonic: *'Little Nancy's Kitchen Runs Crazy Fast'*\n\n**Key Trends:**\n• Atomic radius ↑ down a group\n• Electronegativity ↑ across a period\n• Ionization energy ↑ across a period\n• Metallic character ↑ down a group\n\nValence electrons = Group number (main group)",
  study: "**Smart Study Schedule** 📅\n\n• **6–8 AM:** Revise yesterday's topics\n• **9–12 PM:** New concepts (peak focus time!)\n• **2–5 PM:** Problem solving & DPPs\n• **6–8 PM:** Weak chapter focus\n• **9–10 PM:** Mock test analysis\n\n**Weekly:** 1 full mock every Sunday\n\n🌟 *Golden Rule:* 3 hrs/day × 365 days beats 10 hrs/day × 100 days. Consistency wins!",
  default: "I'm **Obi AI** 🤖 — your 24/7 study companion!\n\nI can help you with:\n• **Doubt solving** — Physics, Chemistry, Maths, Biology\n• **Concept explanations** with examples\n• **Formula quick-reference**\n• **Exam strategy & study plans**\n\nTry asking me: *\"Explain Newton's laws\"* or *\"Integration tricks for JEE\"*",
};

function getAIResponse(msg) {
  const l = msg.toLowerCase();
  if (l.match(/hi|hello|hey|namaste/)) return "Hello! 👋 I'm **Obi AI** from Eduobi. Ask me any doubt — Physics, Chemistry, Maths or Biology. Let's crack JEE/NEET together! 🚀";
  if (l.match(/newton|law.*(motion|physics)|motion.*law/)) return AI_KB.newton;
  if (l.match(/integrat|calculus|differentiat/)) return AI_KB.integration;
  if (l.match(/periodic|element|group.*period|valence/)) return AI_KB.periodic;
  if (l.match(/study|schedule|plan|tips|strategy|routine/)) return AI_KB.study;
  if (l.match(/who.*found|founder|anmol|obi enterprise/)) return "**Eduobi** was founded by **Anmol Thakur**, a visionary educator & entrepreneur. His mission: *no student should quit their dream because of expensive education.* Eduobi is a product of **Obi Enterprises**. 🎓";
  return `Great question about **"${msg}"**! 📚\n\nHere's how to approach it:\n\n1. **Grasp the concept** — Understand definitions & core principles\n2. **Learn the formulas** — Know *when* and *how* to apply them\n3. **Practice problems** — Start easy → JEE/NEET level\n4. **Revise with spaced repetition** — Review after 1 day, 3 days, 1 week\n\nWant me to go deeper on any specific part of this topic? 💡`;
}
...
function AuthModal({ onClose, onLogin, defaultTab = "login" }) {
  ...
  const sendOtp = async (email, name) => {
    ...
    setLoading(false);
    if (sent) {
      setSuccess(`✅ OTP sent to ${email}! Please check your inbox (and spam folder).`);
    } else {
      // Fallback — credentials not set yet
      setSuccess(`⚠️ Email service not configured yet. [DEV OTP: ${code}]`);
    }
      ...
      <div className="modal-box" style={{ maxWidth:420 }} onClick={e => e.stopPropagation()}>
        ...
        {screen === "register" && <>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:13 }}>
            ...
            <ErrBox /><OkBox />
            {/* OTP info banner */}
            <div style={{ background:"rgba(255,107,0,.07)", border:"1px solid rgba(255,107,0,.2)", borderRadius:10, padding:"10px 14px", fontSize:12, color:"#8892A4", lineHeight:1.6 }}>
              📲 OTP will be sent to your <strong style={{ color:"#FF6B00" }}>Email</strong> and <strong style={{ color:"#FF6B00" }}>Mobile</strong> for verification.
            </div>
            <button className="btn-primary" style={{ width:"100%", padding:13, fontSize:15 }} onClick={startRegister} disabled={loading}>
        ...
        {screen === "verify-register" && <>
          <ModalHeader title="Verify Email OTP" subtitle={`OTP bheja gaya: ${form.email}`} back={() => { setScreen("register"); setError(""); }} />
          <div style={{ display:"flex", flexDirection:"column", gap:13 }}>
            <div style={{ textAlign:"center", padding:"20px 14px", background:"rgba(255,107,0,.06)", borderRadius:12, border:"1px solid rgba(255,107,0,.2)" }}>
              ...
              <div style={{ fontSize:13, color:"#8892A4", lineHeight:1.7 }}>
                <strong style={{ color:"#FF6B00" }}>{form.email}</strong> par 4-digit OTP bheja gaya hai. Spam/Promotions folder bhi check karo.
              </div>
            </div>
            ...
            <div style={{ textAlign:"center", fontSize:12, color:"#5A6580" }}>
              OTP nahi mila?{" "}
              <span style={{ color:"#FF6B00", cursor:"pointer", fontWeight:600 }} onClick={() => sendOtp(form.email, form.name)}>Dobara bhejo</span>
            </div>
          </div>
        </>}
...
function ObiAI({ onClose }) {
  const [msgs, setMsgs] = useState([{ role: "ai", text: AI_KB.default }]);
  ...
  const send = (txt) => {
    ...
    setTimeout(() => {
      setTyping(false);
      setMsgs(p => [...p, { role: "ai", text: getAIResponse(m) }]);
    }, 800 + Math.random() * 700);
  };
    ...
    <div className="card" style={{ padding: 0, overflow: "hidden", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: 20, flex: 1, display: "flex", flexDirection: "column" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
          <div style={{ fontSize: 34 }}>{batch.icon}</div>
          {batch.tag && <span className="badge badge-orange">{batch.tag}</span>}
        </div>
        <h3 style={{ fontFamily: "Syne", fontWeight: 700, fontSize: 15, marginBottom: 6, lineHeight: 1.3 }}>{batch.name}</h3>
        <p style={{ color: "#8892A4", fontSize: 12.5, lineHeight: 1.55, marginBottom: 12, flex: 1 }}>{batch.desc}</p>

        <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 12 }}>
          {batch.subjects.map(s => <span key={s} className="badge badge-blue" style={{ fontSize: 10 }}>{s}</span>)}
          <span className="badge badge-green" style={{ fontSize: 10 }}>📅 {batch.duration}</span>
        </div>

        <div style={{ fontSize: 12, color: "#8892A4", marginBottom: 14 }}>
          👨‍🏫 {batch.instructor} &nbsp;·&nbsp; ⭐ {batch.rating} &nbsp;·&nbsp; 👥 {batch.students}
        </div>

        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
          <span style={{ fontFamily: "Syne", fontWeight: 800, fontSize: 22 }}>₹{batch.price.toLocaleString()}</span>
          <span style={{ textDecoration: "line-through", color: "#5A6580", fontSize: 13 }}>₹{batch.originalPrice.toLocaleString()}</span>
          <span className="badge badge-green">{disc}% OFF</span>
        </div>

        <button className="btn-primary" style={{ width: "100%" }} onClick={onEnroll}>Enroll Now</button>
      </div>
    </div>
  );
}
      ...
      <div style={{ background: "linear-gradient(160deg,#0A0A14 0%,#131929 55%,#0A0A14 100%)", padding: "64px 24px 80px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 860, margin: "0 auto", position: "relative" }}>
          ...
          <p style={{ color: "#8892A4", fontSize: 16, marginBottom: 32, maxWidth: 520, lineHeight: 1.75 }}>
            Affordable, world-class education for every Indian student — backed by <strong style={{ color: "#FF6B00" }}>Obi AI</strong>, India's smartest doubt-solver.
          </p>
      ...
      <div style={{ padding: "0 24px 48px", maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
          <div>
            ...
            <p style={{ color: "#8892A4", fontSize: 13, marginTop: 4 }}>Chosen by lakhs of successful students</p>
          </div>
          <button className="btn-ghost" onClick={() => goTo("batches")}>View All →</button>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(300px,1fr))", gap: 20 }}>
          {BATCHES.slice(0, 3).map(b => <BatchCard key={b.id} batch={b} onEnroll={() => user ? alert("Enrolled! (demo)") : openAuth()} />)}
        </div>
      </div>
...
// BATCHES
function BatchesPage({ user, openAuth }) {
  const [filter, setFilter] = useState("all");
  const filtered = filter === "all" ? BATCHES : BATCHES.filter(b => String(b.classId) === filter);

  return (
    <div className="fade-up" style={{ padding: "32px 24px", maxWidth: 1100, margin: "0 auto" }}>
      <h2 style={{ fontFamily: "Syne", fontWeight: 800, fontSize: 28, marginBottom: 6 }}>All Batches</h2>
      <p style={{ color: "#8892A4", marginBottom: 28 }}>Choose your batch and start your journey to success</p>

      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 28 }}>
        {[["all", "All Batches"], ["jee", "JEE"], ["neet", "NEET"], ["11", "Class 11"], ["12", "Class 12"], ["cuet", "CUET"], ["foundation", "Foundation"]].map(([v, l]) => (
          <button key={v} className={`tab-btn ${filter === v ? "active" : ""}`} onClick={() => setFilter(v)}>{l}</button>
        ))}
      </div>

      {filtered.length === 0 && <div style={{ color: "#5A6580", textAlign: "center", padding: 48 }}>No batches found for this filter.</div>}

      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(300px,1fr))", gap: 20 }}>
        {filtered.map(b => (
          <BatchCard key={b.id} batch={b} onEnroll={() => { if (!user) openAuth(); else alert("Enrolled! (demo)"); }} />
        ))}
      </div>
    </div>
  );
...
function LecturesPage({ user, openAuth, awardXP, progress }) {
  ...
  return (
    <div className="fade-up" style={{ padding: "32px 24px", maxWidth: 1100, margin: "0 auto" }}>
      ...
      <p style={{ color: "#8892A4", marginBottom: 28 }}>Watch curated lectures by India's top educators</p>

      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 32 }}>
        {subjects.map(s => <button key={s} className={`tab-btn ${subject === s ? "active" : ""}`} onClick={() => setSubject(s)}>{s}</button>)}
      </div>

      {/* Upload notice for admin */}
      {!user && (
        <div style={{ background: "linear-gradient(135deg,#131929,#1A2440)", border: "1px solid rgba(255,107,0,.2)", borderRadius: 18, padding: "28px 24px", marginBottom: 28, display: "flex", gap: 16, alignItems: "center", flexWrap: "wrap" }}>
          <div style={{ fontSize: 36 }}>🔐</div>
          <div style={{ flex: 1 }}>
            <div style={{ fontFamily: "Syne", fontWeight: 700, fontSize: 17, marginBottom: 6 }}>Login to Access Lectures</div>
            <div style={{ color: "#8892A4", fontSize: 14 }}>Create a free account to unlock lectures added by your batch educators.</div>
          </div>
      ...
      <div style={{ textAlign: "center", padding: "64px 24px" }}>
        ...
        <h3 style={{ fontFamily: "Syne", fontWeight: 700, fontSize: 22, marginBottom: 12 }}>Lectures Coming Soon!</h3>
        <p style={{ color: "#8892A4", lineHeight: 1.8, maxWidth: 440, margin: "0 auto 24px", fontSize: 14 }}>
          Our educators are preparing world-class video lectures for your batch.<br />
          Content will appear here once your batch is activated.<br /><br />
          <strong style={{ color: "#FF6B00" }}>Enroll in a batch</strong> to get notified the moment lectures go live.
        </p>
        <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
          <button className="btn-primary" onClick={() => user ? null : openAuth()}>
            {user ? "Browse Batches" : "Login to Enroll"}
          </button>
          <button className="btn-ghost" onClick={() => document.getElementById("obi-ai-btn").click()}>Ask Obi AI Instead 🤖</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="fade-up" style={{ padding:"32px 24px 60px", maxWidth:1100, margin:"0 auto" }}>
      ...
      <div style={{ background:"linear-gradient(135deg,#131929,#1A2440)", border:"1px solid rgba(155,89,182,.3)", borderRadius:16, padding:"18px 22px", marginBottom:28, display:"flex", alignItems:"center", gap:16, flexWrap:"wrap" }}>
        ...
        <div style={{ flex:1 }}>
          ...
          <div style={{ color:"#8892A4", fontSize:13, marginTop:3 }}>7 years ke solved papers · English, Hindi, Maths, Science, SST</div>
        </div>
      ...
      {!user && (
        <div style={{ background:"linear-gradient(135deg,#131929,#1A2440)", border:"1px solid rgba(255,107,0,.25)", borderRadius:16, padding:"16px 20px", marginBottom:24, display:"flex", gap:14, alignItems:"center", flexWrap:"wrap" }}>
          ...
          <div style={{ flex:1 }}>
            <div style={{ fontWeight:700, fontSize:14, marginBottom:2 }}>Free Login Required</div>
            ...
            <div style={{ padding:"18px 20px", display:"flex", gap:16, alignItems:"center", flexWrap:"wrap" }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:8, flexShrink:0 }}>
                ...
                <button className="btn-secondary" style={{ fontSize:12, padding:"8px 16px", display:"flex", alignItems:"center", gap:6 }}
                  onClick={() => { if(!user){openAuth();return;} alert("PDF download feature — apna PDF link yahan connect karo!"); }}>
                  ⬇️ Download
                </button>
              </div>
            </div>
        ...
        <div className="modal-overlay" onClick={() => setSelectedPaper(null)}>
          <div className="modal-box" style={{ maxWidth:500 }} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{ background:"rgba(255,107,0,.07)", border:"1px solid rgba(255,107,0,.2)", borderRadius:10, padding:"10px 14px", fontSize:12, color:"#8892A4", marginBottom:16 }}>
              🤖 <strong style={{ color:"#FF6B00" }}>Obi Tip:</strong> Pehle last 3 saal ke papers solve karo — pattern samajhna sabse important hai!
            </div>
            <button className="btn-primary" style={{ width:"100%", padding:13 }}
              onClick={() => alert("PDF link yahan add karo — public folder mein PDF daalo!")}>
              ⬇️ Download PDF
            </button>
          </div>
        </div>
    ...
    <div className="fade-up" style={{ padding: "32px 24px 60px", maxWidth: 1100, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 28 }}>
        ...
        <div>
          ...
          <p style={{ color: "#8892A4", fontSize: 13, marginTop: 3 }}>Anmol Thakur ke handwritten notes — bilkul free!</p>
        </div>
      </div>
      ...
      {chapters.length > 0 ? (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 18 }}>
          {chapters.map((ch, i) => (
            <div key={ch.id} className="card" style={{ padding: 0, overflow: "hidden", cursor: "pointer", animation: `fadeUp .35s ease ${i * 0.07}s both` }}
              onClick={() => { if (!user) { openAuth(); return; } setSelectedChapter(ch); }}>
              {/* Thumbnail Banner */}
              ...
              <div style={{ padding: "16px 18px 14px" }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button className="btn-primary" style={{ flex: 1, fontSize: 12, padding: "8px 10px", display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}
                    onClick={e => { e.stopPropagation(); if (!user) { openAuth(); return; } setSelectedChapter(ch); }}>
                    👁️ View Notes
                  </button>
                  {ch.pdfUrl ? (
        ...
        <div style={{ textAlign: "center", padding: "60px 20px" }}>
          ...
          <div style={{ color: "#8892A4", fontSize: 14 }}>{activeSubject} ke notes jald hi available honge. 🙏</div>
        </div>
      )}

      {selectedChapter && <PdfModal chapter={selectedChapter} onClose={() => setSelectedChapter(null)} />}
    </div>
  );
}
    ...
    <div className="fade-up" style={{ padding: "48px 24px 60px", maxWidth: 900, margin: "0 auto" }}>
      ...
      <div className="card" style={{ padding: "28px 32px" }}>
        ...
        <div style={{ borderTop: "1px solid #1A2440", paddingTop: 24, textAlign: "center" }}>
          <div style={{ fontSize: 13, color: "#8892A4", marginBottom: 14 }}>📺 Free lectures, tips & updates pe subscribe karo!</div>
...
export default function App() {
  ...
  const renderPage = () => {
    switch (page) {
      ...
      case "batches": return <BatchesPage user={user} openAuth={openAuth} />;
